...
async function configure (gyp, argv) {
  ...
  async function getNodeDir () {
    ...
    const nodeHeadersDir = process.config.variables.node_headers_dir
    if (!gyp.opts.nodedir &&
      ...
      let availVersion
      try {
        const nodeVersionH = fsSync.readFileSync(path.join(process.config.variables.prefix,
      ...
      if (availVersion === release.version) {
        ...
        gyp.opts.nodedir = nodeHeadersDir
        log.verbose('using local node headers based on node_headers_dir',
